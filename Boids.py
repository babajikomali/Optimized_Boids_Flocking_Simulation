...
PARAMETERS = {'align_radius' : 75, 'cohesion_radius' :  70, 'separation_radius' : 50,
              'align_max_force' : 1.0, 'align_max_speed' : 4.0,'cohesion_max_force' : 0.5, 
              'cohesion_max_speed' : 4.0,'separation_max_force' : 0.8, 'separation_max_speed' : 4.0}
...
def addBoid(boids):
    ...
    velocity = [random.uniform(1.0, 2.0), random.uniform(1.0, 2.0)]
...
while not exit:
    ...
    for i,boid in enumerate(boids):
        ...
        update(boid)
        position = boid[0]
        temp = [position[0],screen_height-position[1]]
        pygame.draw.circle(screen, (255, 255, 255), temp, circle_radius+1)
        pygame.draw.circle(screen, (0,255,0), temp, circle_radius)
